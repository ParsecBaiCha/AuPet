...
watch(experimentViewport, element => {
  ...
  if (!element) return
  const resize = () => { boardScale.value = Math.min(element.clientWidth / 1000, element.clientHeight / 640) }
...
<template>
  <section class="deep-lesson" :class="{ 'compact-lesson': !experiment, 'single-screen-experiment': experiment }">
    <template v-if="!experiment">
      <header><h3>{{ lesson.title }}</h3></header>
...
<style scoped>
.compact-lesson { gap: clamp(8px, 1.5cqh, 14px); color: #414651; }
.compact-lesson header h3 { font-size: clamp(22px, 3.5cqh, 30px); margin: 0; }
.compact-lesson .steps { display: grid; grid-template-columns: repeat(6, minmax(0, 1fr)); }
.compact-lesson .steps button { padding: 8px 6px; font-size: 14px; border-radius: 8px; }
.compact-lesson .steps button.active { background: #fff0cc; border-color: #efb348; color: #75501b; font-weight: 600; }
.compact-lesson .formula { flex-basis: 20%; background: #f7f5fb; padding: 14px 20px; }
.compact-lesson .formula small { font-size: 13px; color: #756884; }
.compact-lesson .formula code { color: #493d65; font-size: clamp(19px, 3.4cqh, 28px); margin-top: 6px; }
.compact-lesson .explanation { gap: 12px; }
.compact-lesson .explanation article { padding: clamp(12px, 2cqh, 20px); gap: 8px; border-color: #e8e1d8; }
.compact-lesson .explanation h4 { font-size: clamp(16px, 2.5cqh, 20px); color: #353e49; }
.compact-lesson .explanation p { font-size: clamp(15px, 2.3cqh, 19px); line-height: 1.6; color: #4f535a; }
@media (max-width: 800px) { .compact-lesson .steps { grid-template-columns: repeat(3, minmax(0, 1fr)); } }
</style>

<style scoped>
/* A balanced four-panel experiment, preserving the fixed-coordinate SVG model. */
.experiment-board { height: 640px; gap: 10px; color: #374151; }
.experiment-board .experiment-head { height: 64px; }
.experiment-board .experiment-head .eyebrow { display: none; }
.experiment-board header h3 { font-size: 24px; color: #303946; margin: 0 0 6px; }
.experiment-board header p { font-size: 13px; color: #626976; }
.experiment-board .experiment-head-actions { gap: 8px; }
.experiment-board .experiment-head-actions > button:not(.experiment-close) { border-radius: 8px; }
.experiment-board .lab-grid { grid-template-columns: repeat(10, minmax(0, 1fr)); grid-template-rows: 0.95fr 1.05fr; gap: 12px; }
.experiment-board .module:nth-child(1), .experiment-board .module:nth-child(3) { grid-column: span 4; }
.experiment-board .module:nth-child(2), .experiment-board .module:nth-child(4) { grid-column: span 6; }
.experiment-board .module { border-color: #e6dfd4; border-radius: 12px; padding: 12px 14px; box-shadow: 0 3px 12px #604a2305; }
.experiment-board .module h4 { font-size: 16px; color: #35404d; margin-bottom: 10px; padding-bottom: 8px; border-bottom: 1px solid #f0eae1; }
.experiment-board .data-layout { gap: 14px; }
.experiment-board .scatter { width: 172px; height: 172px; border-radius: 9px; }
.experiment-board .legend { font-size: 13px; color: #4b5563; }
.experiment-board .legend strong { display: block; background: #f5effb; border-radius: 7px; padding: 7px 8px; color: #705090; font-size: 14px; }
.experiment-board .model-module { grid-template-columns: 105px minmax(0, 1fr); }
.experiment-board .model-module > .network { transform: none; }
.experiment-board .model-flow { gap: 4px; }
.experiment-board .model-flow span { padding: 7px 5px; background: #f8f5fc; }
.experiment-board .model-flow b { font-size: 12px; }
.experiment-board .model-flow small { font-size: 10px; }
.experiment-board .model-hint { background: #faf8f4; font-size: 11px; color: #62605c; }
.experiment-board .settings-module .controls { gap: 5px 10px; }
.experiment-board .settings-module .controls label { font-size: 12px; gap: 4px; color: #525a65; }
.experiment-board .settings-module .controls select { padding: 5px 7px; font-size: 12px; border-color: #e4dccf; border-radius: 7px; background: #fffdf9; }
.experiment-board .settings-module .status { background: #fff5df; border: 1px solid #f4dfb5; border-radius: 8px; padding: 5px 10px; color: #89601f; font-size: 12px; line-height: 1.5; }
.experiment-board .settings-module .actions { gap: 8px; margin-top: 10px; }
.experiment-board .settings-module .actions button { font-size: 13px; padding: 7px 12px; border-radius: 8px; }
.experiment-board .settings-module .actions .primary,
.experiment-board .experiment-expand { background: #ffcc63; border-color: #f3b53d; color: #694515; font-weight: 600; }
.experiment-board .settings-module .actions .primary:hover:not(:disabled),
.experiment-board .experiment-expand:hover:not(:disabled) { background: #ffbd40; color: #694515; border-color: #e6a127; }
.experiment-board .settings-module > .hint { font-size: 11px; color: #766b5d; padding-top: 8px; }
.experiment-board .charts { gap: 8px; }
.experiment-board .chart { background: #fdfcf9; border-color: #ebe4db; border-radius: 9px; }
.experiment-board .chart h5 { font-size: 14px; color: #4c4659; }
.experiment-board .chart p { font-size: 12px; }
.experiment-board .reflection { font-size: 12px; color: #77674e; background: #fff8ea; padding: 7px 12px; border-radius: 8px; }
.compact-lesson .steps button { min-height: 38px; }
.compact-lesson .explanation .answer { border-left: 3px solid #efbb5a; border-radius: 0 8px 8px 0; }
</style>
