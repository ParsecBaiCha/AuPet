...
const form = ref<{ id?: number; title: string; content: string; author: string; type: string; status: string; isTop: boolean }>({
  title: '', content: '', author: '',
  type: 'daily', status: 'pending', isTop: false
})
...
const fetchPosts = async () => {
  loading.value = true
  try {
    const res = await adminApi.getDailyForumPosts()
...
const filterPosts = computed(() => {
  let result = posts.value.filter(p => p.type === 'daily')
...
const openDialog = () => {
  form.value = { title: '', content: '', author: '', type: 'daily', status: 'pending', isTop: false }
