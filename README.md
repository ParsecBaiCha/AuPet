...
- **AI 智能伙伴**：基于大语言模型（DeepSeek）的年级自适应对话，提供 AI 通识课教学、出题测验、故事绘本、动画讲解
...
| **大语言模型** | DeepSeek API | 对话、出题、故事绘本、动画代码生成 |
...
详细的接口清单与数据结构请参阅：
- [后端接口清单分析报告.md](./后端接口清单分析报告.md)
- [后端接口清单与数据结构分析报告.md](./后端接口清单与数据结构分析报告.md)
